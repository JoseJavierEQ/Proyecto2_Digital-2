...
const int joystickSel1 = PD_0;     // the number of the joystick select pin
...
// SD
File myFile;
extern uint8_t bomberman1[];
extern uint8_t bomberman2[];
...
void setup() {
  ...
  pinMode(joystickSel1, INPUT_PULLUP);
  ...
  LCD_Init();
  LCD_Clear(0x00);
  FillRect(0, 0, 319, 239, 0x8cc6ff);

  //*****************************************************
  myFile = SD.open("bomber~1.txt");
  uint16_t n = 0;
  ...
  if (myFile) {
    ...
    while (myFile.available()) {
      ...
      for (uint8_t m = 0; m < 2; m++) {
        char valor = myFile.read();

        unsigned char digito = Char_to_uChar(valor);
  ...
  LCD_Bitmap(logox, logoy, width, height, vegueta);

  String textoinit = "Cargando...";
  LCD_Print(textoinit, 130, 188, 1, 0x000000, 0x8cc6ff);
...
//    }
    ...
    if (borrar != 0) {
      ...
      if (borrar > 2) {
        ...
        if ((Bombaexplota == 3) && (Bombaexplota == 2)) {
          FillRect(x2, y2, 28, 36, 0x8cc6ff);
          x2 = 281;
          y2 = 192;
          LCD_Sprite(x2, y2, 28, 36, bomberman2, 3, anim, 0, 0);
    ...
    joystickSelState1 = digitalRead(joystickSel1);
    if (joystickSelState1 == LOW) {
      bx2 = x2 + 1;
      by2 = y2 + 5;
      b5 = 1;
      Explo5 = 1;
  ...
  FillRect(0, 0, 319, 239, 0x8cc6ff);
  if (Bandera == 0) {
    LCD_Sprite(146, 102, 28, 36, bomberman1, 3, 1, 0, 0);
    String textoJ1 = "Ganador: J1";
    LCD_Print(textoJ1, 130, 188, 1, 0x000000, 0x8cc6ff);
  }
  if (Bandera == 1) {
    LCD_Sprite(146, 102, 28, 36, bomberman2, 3, 1, 0, 0);
    String textoJ2 = "Ganador: J2";
    LCD_Print(textoJ2, 130, 188, 1, 0x000000, 0x8cc6ff);
  }
  delay (2000);
  FillRect(0, 0, 319, 239, 0x8cc6ff);
  initTimer();
